...
def insert_picture(text, imgs, id):
    ...
    for image in imgs:
        ...
        html_img_tag = (
            f'<img src="data:image/png;base64,{base64_encoded}" alt="《大陸居民臺灣正體字講義》一簡多繁辨析條目「'
            + id
            + "」疑難雜字("
            + str(index)
            + '")>'
        ...
        if all_pinyin[i] not in converter_dictionary:
            ...
            if pinyin_comp != all_pinyin[i]:
                print(
                    "Error: pinyin",
                    all_pinyin[i],
                    "might be confused and should be writed as",
                    pinyin_comp,
                    ".",
                )
                exit()
...
def write_converter_log(filename):
    ...
    with open(filename, mode="w", encoding="utf-8") as csv_file:
        ...
        converter_log = reversed(
            sorted(
                list(converter_dictionary.keys()),
                key=functools.cmp_to_key(compare_pinyin),
            )
        )
